...
# Paths
input_pdf_folder = r"D:\Projects\BPGscript\trial_pdfs"
...
for pdf_file in os.listdir(input_pdf_folder):
    ...
    for pdf_path in individual_page_paths:
        document_name = pdf_file  # Use original PDF name for grouping

        # Extract actual page number from filename
        match = re.search(r'_page_(\d+)\.pdf$', pdf_path)
        page_number = int(match.group(1)) if match else -1

        try:
            with pdfplumber.open(pdf_path) as pdf:
                for page in pdf.pages:  # only one page per split file
...
# --- ✅ Postprocessing: Backfill document-level fields ---
doc_level_fields = ["Processor Name", "Payer Name", "Payer Parent Name", "Effective Date"]
